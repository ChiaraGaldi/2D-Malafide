...
def test():
    ...
    with open(os.path.join(results_path, 'val_only_results_w_sbi.txt'), 'w') as file:
    ...
    with open(os.path.join(results_path, 'val_only_results_w_sbi.txt'), 'a') as file:
        file.write(f"\nTEST RESULTS:\nFrame-EER of {cfg['dataset']['name']} is {eer:.4f}\nFrame-AUC of {cfg['dataset']['name']} is {f_auc:.4f}\nFrame-attack rate of {cfg['dataset']['name']} is {val_attack_rate:.4f}")
...
def save_sample(batch, video_name, description, results_path):
    ...
    if batch.size(0) > 1:
        for i in range(batch.size(0)):
            ...
            img_save_path = os.path.join(results_path, 'imgs_sbi')
    ...
    with torch.no_grad():
        for (batch_data, batch_labels) in loop:
            ...
            if batch_data_spoof.size(0) > 0:
                ...
                #save_sample(batch_data_spoof, video_name, description, img_save_path)
