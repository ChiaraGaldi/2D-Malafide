...
def test(black_box):
    ...
    # Initialize Grad-CAM
    #target_layers=[cm_model.module.base_model._blocks[-1]._project_conv]
    #target_layers=[cm_model.module.adm.adm_extra_layers[-1].downsample]
    target_layers=[cm_model.module.base_model._conv_head]
    cam = GradCAM(model=cm_model.module, target_layers=target_layers)
    ...
    if black_box:
        SBI_malafide_model_load_path = os.path.join(cfg['malafide']['pretrained'], attack_type, str(malafide_filter_size), 'weights', 'best_filter.pth') 
...
def save_gradcam(grayscale_cam, batch_data, batch_video_name, label, cam_save_path):
    for i in range(len(grayscale_cam)):
        original_img = np.float32(batch_data[i, :, :, :].cpu().detach().numpy().transpose(1, 2, 0))/255
        cam_image = show_cam_on_image(original_img, grayscale_cam[i], use_rgb=True)
...
def validation_epoch(
    ...
    for (batch_data, batch_labels) in loop:
        ...
        if batch_data_bonafide.size(0) > 0:
            ...
            grayscale_cam = cam(input_tensor=batch_data_bonafide, targets=cam_targets)
        ...
        if batch_data_spoof.size(0) > 0:
            ...
            grayscale_cam = cam(input_tensor=batch_data_spoof, targets=cam_targets)
