...
def test():
    ...
    with open(os.path.join(results_path, 'results_validation_only_sbi.txt'), 'w') as file:
    ...
    malafide_model_load_path = os.path.join(malafide_model_save_path, 'best_filter.pth') 
    ...
    with open(os.path.join(results_path, 'results_validation_only_sbi.txt'), 'a') as file:
        file.write(f"\nTEST RESULTS:\nFrame-EER of {cfg['dataset']['name']} is {eer:.4f}\nFrame-AUC of {cfg['dataset']['name']} is {f_auc:.4f}\nFrame-attack rate of {cfg['dataset']['name']} is {val_attack_rate:.4f}")
